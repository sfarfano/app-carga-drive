...
def subir_a_drive(nombre_archivo, archivo_stream, tipo_mime, carpeta_id):
    # Validar si ya existe el archivo con ese nombre en la carpeta
    existentes = listar_archivos(carpeta_id)
    if any(f['name'] == nombre_archivo for f in existentes):
        st.warning("⚠️ Ya existe un archivo con ese nombre en la carpeta. Cambia el nombre o elimina el anterior.")
        st.stop()
    ...
    if not proyecto or not sub1:
        ...
        if nombres:
            ...
            if st.button("❌ Eliminar archivo"):
                if eliminar_archivo(nombres[seleccion]):
